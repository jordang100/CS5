...
def writefile(machinecode, filename) :
    ...
    for triplet in machinecode:
        print(textwrap.wrap((str(triplet[0])).ljust(nwidth) + " : " + (triplet[1]).ljust(27) + triplet[2], 100)[0])
...
def main(programAsString = None) :
    ...
    if (not machinecode == []) and (not failure):
        ...
        # Go!
        # import hmmmSimulator ; imp.reload(hmmmSimulator)
        # hmmmSimulator.main()
